...
Mesh::Mesh(std::vector<Vertex> _meshVertices, std::vector<GLuint> _meshIndices)
{
	meshVertices = _meshVertices;
...
Mesh::Mesh(std::vector<Vertex> _meshVertices, std::vector<GLuint> _meshIndices, std::vector<Texture> _meshTextures)
{
	meshVertices = _meshVertices;
	meshIndices = _meshIndices;
	meshTextures = _meshTextures;
	UploadElemTexData(meshVertices, meshIndices, meshTextures);
}
...
void Mesh::UploadElementsData(std::vector<Vertex> _meshVertices, std::vector<GLuint> _meshIndices)
{
	meshVertices = _meshVertices;
	meshIndices = _meshIndices;
...
void Mesh::UploadElemTexData(std::vector<Vertex> _meshVertices, std::vector<GLuint> _meshIndices, std::vector<Texture> _meshTextures)
{
	meshVertices = _meshVertices;
	meshIndices = _meshIndices;
	meshTextures = _meshTextures;
...
void Mesh::DrawElemTriMesh(glm::mat4 model)
{
	glBindVertexArray(vao);
...
void Mesh::DrawElemTexMesh(glm::mat4 model)
{
	for (GLuint i = 0; i < meshTextures.size(); i++) {
		glActiveTexture(GL_TEXTURE0 + i);
		glBindTexture(GL_TEXTURE_2D, meshTextures[i].textureIndex);
	}
